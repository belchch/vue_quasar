<template>
  <div>
    <q-table :rows="sources" :columns="columns" separator="cell" hide-pagination flat bordered hide-header dense>
      <template v-slot:top-row>
        <q-tr>
          ...
          <q-td>
            <q-input v-model.number="price" input-class="text-right" type="number" dense borderless />
          </q-td>
      ...
      <template v-slot:body-cell-url="props">
        <q-td :props="props">
          <q-input @keydown.enter.prevent="$event.target.blur()" @blur="submit(props.row, 'url')"
            @focus="backupFieldValue(props.row, 'url')" v-model="props.row.url" input-class="text-right" dense
            borderless />
        </q-td>
      </template>
      <template v-slot:body-cell-price="props">
        <q-td :props="props" style="width:100px">
          <q-input @blur="submit(props.row, 'price')" @keydown.enter.prevent="$event.target.blur()"
            @focus="backupFieldValue(props.row, 'price')" v-model.number="props.row.price" input-class="text-right"
...
const submit = async (row:Sources, fieldName: FieldName) => {
  try{
    ...
    if (updatedRate.boqWorkParams['id']) { delete updatedRate.boqWorkParams.id }
    delete updatedRate['boqWorkParams']['material']
    await rateStore.updateRate(rate);
  }catch{
