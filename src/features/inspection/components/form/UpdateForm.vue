  ...
  <q-card-section v-if="localCase">
    <q-form class="q-gutter-sm" @submit="handleSave">
      <div class="row q-col-gutter-lg">
        <div class="col-6">
          <FormInput v-model="localCase.number" label="Номер" title="Дело (договор)" :required="true" />
            ...
            <q-input dense outlined readonly v-model="startDate">
              ...
              <template v-slot:append>
                <q-btn icon="access_time" flat no-caps round2 class="bg-grey-3 text-grey-8" style="margin: 1px -11px"
                  size="md">
                  <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                    <q-time v-model="localCase.inspectionStartAt" mask="YYYY-MM-DDTHH:mm:ssZ" format24h>
                      <div class="row items-center justify-end">
                        <q-btn v-close-popup label="Закрыть" color="primary" flat />
                      </div>
                    </q-time>
