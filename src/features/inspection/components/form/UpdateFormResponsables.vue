      ...
      <div class="row q-col-gutter-lg">
        <div class="col-6">
          <FormList :disable-permission="!hasPermission(['case.update.manager'])" label="Менеджер"
            v-model="localCase.manager" :options="managerOptions" option-label="label" option-value="value" emit-value
            map-options />
        </div>
        <div class="col-6">
          <FormList :disable-permission="!hasPermission(['case.update.head'])" label="Руководитель"
            v-model="localCase.head" :options="headOptions" option-label="label" option-value="value" emit-value
            map-options />
        </div>
      </div>
      <div class="row q-col-gutter-lg">
        ...
        <div class="col-6">
          <FormList :disable-permission="!hasPermission(['case.update.expert'])" label="Эксперты" :multiple="true"
            v-model="localCase.experts" :options="expertOptions" option-label="label" option-value="value" emit-value
            map-options />
        </div>
      </div>
