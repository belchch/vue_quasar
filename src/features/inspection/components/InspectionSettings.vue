    ...
    <div class="row justify-between">
      <q-card-section>
        <div class="q-pa-sm row text-subtitle2 text-accent">
          <div v-if="selectedInspection" class="cursor-pointer">
            ...
            <q-popup-edit
              v-model="selectedInspection.apartment"
              auto-save
              v-slot="scope"
              buttons
              @save="saveApartment"
      ...
      <q-card-section>
        <div class="q-gutter-md">
          <q-btn
            v-if="canDelete && hasPermission(['inspection.delete'])"
            color="negative"
            size="sm"
            icon="delete"
            @click="deleteInspection"
            :loading="deleteInProgress"
            >Удалить осмотр</q-btn
          >
        </div>
      </q-card-section>
    </div>
...
const newSpots = (): InspectionSpot[] => {
  return spotStore.items
    ?.filter((item) => !inspectionSpots.value?.some((is: InspectionSpot) => is.spot.id == item.id))
    .map((item) => ({
      spot: item,
      count: 1,
      inUse: false,
