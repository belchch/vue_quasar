<template>
  <div class="q-pt-sm">
    <q-table :rows="estimateStore.estimate?.groups || []" :rows-per-page-options="[0]" :row-key="(row) => getKey(row)"
...
import { EstimateGroup } from 'src/features/estimate/api/types'
...
const getKey = (row:EstimateGroup) => {
  return estimateStore.estimate?.groups.indexOf(row) + '_' + row.type;
}
