...
const props = defineProps<{
  ...
  usePhotoApi: (id: number, use: boolean, scoped: number) => Promise<DefectReportPhotoModel[]>
}>()

const photosLocal = ref(props.photos)

const togglePhoto = async (id: number, use: boolean) => {
  photosLocal.value = await props.usePhotoApi(id, use, 2)
}

const showUrls = computed(() =>
  photosLocal.value.filter((item) => item.used).map((item) => item.url),
)
</script>
<style scoped></style>
