<template>
  ...
  <div v-if="docs.length == 0" class="text-h5 text-weight-light text-center">
    {{ emptyDocsLabel }}
  </div>
      ...
      >
        ...
        <div class="hover-controls absolute-top column" v-if="hasPermission(['document.update'])">
          <q-btn flat text-color="white" icon="edit" class="hover-delete-btn" @click="edit(doc)">
            <q-tooltip :anchor="'center right'" :self="'center start'"> Редактировать </q-tooltip>
          </q-btn>
          <q-btn
            flat
            text-color="white"
          ...
          >
            <q-tooltip :anchor="'center right'" :self="'center start'"> Скачать </q-tooltip>
          </q-btn>
          <q-btn
            flat
            text-color="white"
          ...
          >
            <q-tooltip :anchor="'center right'" :self="'center start'"> Удалить </q-tooltip>
          </q-btn>
        </div>
        <q-separator />
...
const initialCardSize = computed(() => {
  const range = sliderRange.value
  return Math.round((range.min + range.max) / 2)
})

const cardSize = ref(initialCardSize.value)
...
const downloadDoc = async (doc: CaseDocumentResponse) => {
  try {
    ...
    if (fileNameSplit.length > 1) fileExt = '.' + fileNameSplit.pop() || 'unknown'
