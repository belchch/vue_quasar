<template>
  <q-dialog v-model="open">
    <q-uploader :factory="uploadFactory" method="PUT" :send-raw="true" class="img-uploader"
      @uploaded="onUploaded" @factory-failed="onFactoryFailed" thumbnail-fit="contain" 
      accept=".csv" label="Загрузка CSV"
      />
  </q-dialog>
  <q-btn color="primary" icon="upload" label="Загрузить" @click="onOpenUpload"/>
</template>

<script setup lang="ts">
import { ref } from 'vue'
import { useUploadLookup } from '../composables/useUploadLookup'
import { LookupApi } from '../api/LookupApi'

const open = ref(false)
const uploadLookup = useUploadLookup()

const onOpenUpload = () => {
  open.value = true
}

const onUploaded = async (info: any) => {
  const objectKey = info.files[0].objectKey
  await LookupApi.uploadLookup(objectKey)
}

const onFactoryFailed = (error: any) => {
  console.error('error', error)
}

const getExtension = (filename: string) => {
  const lastDotIndex = filename.lastIndexOf('.')
  return lastDotIndex === -1 ? '' : filename.substring(lastDotIndex + 1)
}

const uploadFactory = async (files: any) => {
  const file = files[0]
  console.log('length', files.length)
  console.log('file', file)
  const { url, objectKey } = await uploadLookup(getExtension(file.name))
  file.objectKey = objectKey

  return {
    url,
  }
}
</script>

<style lang="scss">
.img-uploader {
  max-width: 100% !important;
  width: 800px;

  .q-uploader__list {
    display: grid;
    grid-template-columns: repeat(auto-fill, minmax(200px, 1fr));
    gap: 16px;
  }
}
</style>
