      ...
      <q-card style="width: 100%;max-width: 500px;">
        ...
        <q-form @submit.prevent="onConfirmClick" ref="formRef">
          ...
          <q-card-section class="q-pt-none">
            ...
            <q-input outlined dense v-model.number="objectForm.width" label="Ширина" :rules="widthRules"/>
          </q-card-section>
          <q-card-section v-if="sectionType != 'wall_section'" class="q-pt-none">
            <div class="text-subtitle1 q-mb-sm">Длина</div>
            <q-input outlined dense v-model.number="objectForm.length" label="Длина" :rules="lengthRules" />
          </q-card-section>
          <q-card-section v-if="sectionType == 'wall_section' || sectionType == 'fixed_asset'" class="q-pt-none">
            <div class="text-subtitle1 q-mb-sm">Высота</div>
            <q-input outlined dense v-model.number="objectForm.height" label="Высота" :rules="heightRules" />
          </q-card-section>
          <q-card-actions align="right">
            <q-btn label="Отмена" type="button" color="primary" v-close-popup flat class="q-ml-sm" />
...
const { btnText = 'Добавить', room, sectionType, roomNum } = defineProps<{
  btnText?: string,
  room: Spot,
...
// Правила валидации
const nameRules = [
  (val: string) => sectionType !== 'fixed_asset' || (val && val.trim().length > 0) || 'Поле обязательно для заполнения'
];

const requaredRules = (v:any) => v !== null && v !== undefined && v !== '' || 'Поле обязательно для заполнения';
const positiveRules = (v:number) => !isNaN(v) && v > 0 || 'Должно быть положительным числом';

const materialRules = [
  (val: Material) => sectionType === 'fixed_asset' || val !==null && val !== undefined || 'Поле обязательно для заполнения',
];

const widthRules = [
  (val: number) => requaredRules(val),
  (val: number) => positiveRules(val)
];

const heightRules = [
  (val: number) => sectionType !== 'wall_section' && sectionType !== 'fixed_asset' || requaredRules(val),
  (val: number) => sectionType !== 'wall_section' && sectionType !== 'fixed_asset' || positiveRules(val),
]
...
const onConfirmClick = async () => {
  try{
    ...
    if (sectionType === 'floor_section') await measurementService.createFloorSectionMeasurement(request);
    if (sectionType === 'ceil_section') await measurementService.createCeilSectionMeasurement(request);
    if (sectionType === 'wall_section') await measurementService.createWallSectionMeasurement(request);
    loading.value = false
