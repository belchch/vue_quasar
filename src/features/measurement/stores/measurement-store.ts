...
export const useMeasurementStore = defineStore('room-measurements', () => {
  ...
  const allRoomMeasurements = computed(() => {
    ...
    inspectionSpots.value
      .filter((item) => item.inUse && item.spot.isRoom)
      .forEach((item) => {
  ...
  const getRoomCeilSections = computed(() => {
    return (roomId: number, roomNum?: number) => {
      return _findItemsByIdAndNum(ceilSectionMeasurements.value, roomId, roomNum) as
        | CeilSectionMeasurement[]
        | []
    }
  })
  ...
  const getFloorCeilSections = computed(() => {
    return (roomId: number, roomNum?: number) => {
      return _findItemsByIdAndNum(
        floorSectionMeasurements.value,
        roomId,
        roomNum,
  ...
  const getWallCeilSections = computed(() => {
    return (roomId: number, roomNum?: number) => {
      return _findItemsByIdAndNum(
        wallSectionMeasurements.value,
        roomId,
        roomNum,
      ) as WallSectionMeasurement[]
    }
  })
