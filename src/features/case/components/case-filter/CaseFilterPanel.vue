<template>
  <q-card flat bordered square2 class="row q-gutter-y-md shadow_custom"
    style="position: sticky; top: 48px; margin-left: -1px; overflow: hidden">

    <q-input dense v-model="search" placeholder="По номеру или адресу" outlined2 square
      class="filter-search-input q-ma-none full-width">
      <template v-slot:append>
        <q-icon v-if="!_.isEmpty(filter.search)" name="close" @click="search = undefined" class="cursor-pointer" />
    ...
    <div class="row no-wrap q-gutter-md full-width q-mt-none" style="position: relative">
      ...
      <OptionGroupFilter :options="priorityOptions" title="Приоритет" v-model="selectedPriorities" />
    </div>
    <q-card-section class="q-pb-none q-px-none q-ma-none full-width">
      <q-separator />
      <q-card-section class="row justify-between2">
        <DateFilter title="Дата создания" v-model="createdAt" @update:model-value="updateCreatedAt" />
    ...
    <q-card-section class="row justify-center full-width q-gutter-x-md q-mt-none q-pt-none">
      <q-btn no-caps outline color="secondary" icon="close" @click="clearFilters()" label="Сбросить" />
